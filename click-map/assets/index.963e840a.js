...
const attachTransparentClass = (element) => {
  ...
  if (element.shadowRoot) {
    ...
    styleAttr = [
      styleAttr,
      "--ion-background-color: var(--mkg-background-color) !important;",
      "--background-color: var(--mkg-background-color) !important;",
      "--background: var(--mkg-background-color) !important;"
    ].join("");
...
const getPluginDomId = (element) => {
  ...
  if (!elemId) {
    elemId = element === document.body ? "root" : `mkg${Math.floor(Math.random() * Date.now())}`;
...
class HTMLElementBase extends HTMLElement {
  ...
  async onConnected() {
    await window.customElements.whenDefined("mkg-mapview");
...
class LatLng extends HTMLElementBase {
  ...
  _onPropertyChanged() {
    ...
    this._timer = setTimeout(this._update, 5);
  }
  _onUpdate() {
    this._timer = null;
    if (!this._changed) {
...
const detachTransparentClass = (element) => {
  ...
  if (element.shadowRoot) {
    let styleAttr = element.getAttribute("style") || "";
    if (styleAttr && styleAttr.indexOf("--mkg-background-color") > -1) {
      styleAttr = styleAttr.replace("--ion-background-color: var(--mkg-background-color) !important;", "").replace("--background-color: var(--mkg-background-color) !important;", "").replace("--background: var(--mkg-background-color) !important;", "");
    }
    element.setAttribute("style", styleAttr);
